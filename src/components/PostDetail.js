...
function PostDetail() {
    ...
    return (
        <div className="bg-black p-6">
            <div className="fixed top-0 left-0 right-0 bg-black pt-4 pl-16 pr-16">
                <div className="grid grid-flow-col auto-cols-max gap-2 mb-4 items-center">
                    <Avatar
                        src={"https://avatars.githubusercontent.com/u/77449569?v=4"} // Use actual author image URL
                        alt={selectedPost.author || 'Author'}
                        borderColor="border-green-800"
                        border={true}
                    />
                    <div>
                        <div className="text-white">{selectedPost.author || 'Unknown Author'}</div>
                        <div className="text-white">{selectedPost.date || 'Date not available'}</div>
                    </div>
                </div>
                <DividerWithText
